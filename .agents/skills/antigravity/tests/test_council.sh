...
test_council_both_voices_ok() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" cr1 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_partial_when_one_absent() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" cr2 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_fails_when_both_absent() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" cr3 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_redacts_secrets_before_persisting() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" cr4 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_refuses_missing_blob() {
  local r; r=$(mk_repo)
  bash "${SKILL_TIERS:-}/council.sh" cr5 "/no/such/blob/file" --repo "$r" >/dev/null 2>&1
  local code=$?
...
test_council_is_hardened() {                       # source-assertion (drives real CLIs)
  local src="${SKILL_TIERS:-}/council.sh"
...
test_council_copilot_ok_when_healthy() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COPILOT_BIN="$bin/copilot" COUNCIL_INCLUDE_COPILOT=1 \
    bash "${SKILL_TIERS:-}/council.sh" crc1 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_baseline_survives_copilot_absent() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COPILOT_BIN="$bin/copilot" COUNCIL_INCLUDE_COPILOT=1 \
    bash "${SKILL_TIERS:-}/council.sh" crc2 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_copilot_skipped_when_disabled() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" crc3 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_quorum_unaffected_by_copilot() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COPILOT_BIN="$bin/copilot" COUNCIL_INCLUDE_COPILOT=1 \
    bash "${SKILL_TIERS:-}/council.sh" crc4 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_copilot_preflight_gates_p_call() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COPILOT_BIN="$bin/copilot" COUNCIL_INCLUDE_COPILOT=1 COPILOT_SENTINEL="$sentinel" \
    bash "${SKILL_TIERS:-}/council.sh" crc5 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_agy_model_pinned_correctly() {       # source-assertion (regression guard)
  ...
  local b; b="$(cat "${SKILL_TIERS:-}/council.sh" 2>/dev/null)"
...
test_council_agy_disabled_never_invokes_binary() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_AGY=0 COUNCIL_INCLUDE_COPILOT=0 AGY_SENTINEL="$sentinel" \
    bash "${SKILL_TIERS:-}/council.sh" crc6 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
  assert_eq "$([[ -e "$sentinel" ]] && echo invoked || echo not-invoked)" "not-invoked" \
...
test_council_agy_default_on_unchanged() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" crc7 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_quorum_correct_when_agy_disabled() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_AGY=0 COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" crc8 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_codex_oss_disable_falls_back_to_cloud() {
  ...
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 CODEX_USE_OSS=0 \
    bash "${SKILL_TIERS:-}/council.sh" crc10 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
...
test_council_quota_diagnostic_hints() {
  ...
  local blob; blob=$(mktemp); echo "diff" > "$blob"
  CODEX_BIN="$bin/codex" AGY_BIN="$bin/agy" COUNCIL_INCLUDE_COPILOT=0 \
    bash "${SKILL_TIERS:-}/council.sh" crc9 "$blob" --repo "$r" --timeout 30 >/dev/null 2>&1
