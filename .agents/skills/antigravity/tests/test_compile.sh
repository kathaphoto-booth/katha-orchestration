#!/usr/bin/env bash
# Tests for bin/compile-memory.sh atomicity + self-locking.
#
# This is a deliberately static/structural test — grepping the source for the
# right primitives — not a dynamic behavioral test. compile-memory.sh hardcodes a
...
test_compile_never_leaves_partial() {
  ...
  assert_contains "$(cat /Users/jedg./Desktop/kat_ha_pb/bin/compile-memory.sh)" 'mktemp' "compile uses temp file"
  assert_contains "$(cat /Users/jedg./Desktop/kat_ha_pb/bin/compile-memory.sh)" 'mv ' "compile renames atomically"
  assert_contains "$(cat /Users/jedg./Desktop/kat_ha_pb/bin/compile-memory.sh)" '.sync.lock' "compile self-locks"
}
