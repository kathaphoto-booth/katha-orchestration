...
- **Recommended flow:** the custom site is the branded top-of-funnel (browse tiers, check availability, design the card). The final **"Secure Dates"** action **hands the inquiry to HoneyBook's embedded Lead Form** (embed it in the final step, or open it prefilled), so HoneyBook fires its own invoice/contract/payment automation. The site **also** logs the lead to Supabase (your record + the availability admin). Net: gorgeous UX up front, HoneyBook owns the transaction.
