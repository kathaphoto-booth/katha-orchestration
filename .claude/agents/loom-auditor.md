...
- DESIGN.md (root) + vault patterns.md (truth — patterns.md wins any conflict)
...
2. Grep the HTML for:
   ...
   - Forbidden vocab (full list from DESIGN.md §6 + vault patterns.md)
   - Font drift (Cormorant, missing Fraunces/EB Garamond)
   - `<hr>`, `rounded-`, `6/6`, `grid-cols-12`
   - Hex census of all 10 brand tokens + 2 ecru-safe text tokens — flag any token with 0 occurrences AND any `#8C382A` Loko Rust count > 5 (sacred CTA dilution)
3. Read the screenshot. Look specifically for:
   - KathaThread (continuous Piña Ecru SVG line down the page)
   - Wordmark/logomark placement consistent with the two-marks lock (patterns.md)
